...
{
    public sealed class YutuziSizeNamer : IFileNamer<PKM>
    {
        ...
        private static string GetRegular(PKM pk)
        {
            ...
            string size = GetScale(pk).Length > 0 ? $"- {GetScale(pk)} " : string.Empty;
            ...
            if (pk.IsEgg)
                return $"{pk.Species:000}{form}{shinytype} - {speciesName}{formName}{rareForm} ({ballFormatted}) {gender}{ability} - {nature} - {IVList} {size}- {pk.OT_Name}";

            if (pk.IsNicknamed)
                return $"{pk.Species:000}{form}{shinytype} - {pk.Nickname} [{speciesName}{formName}{rareForm} ({ballFormatted})] {gender}{ability} - {nature} - {IVList} {size}- {pk.OT_Name}";
            else
                return $"{pk.Species:000}{form}{shinytype} - {speciesName}{formName}{rareForm} [{speciesName}{formName} ({ballFormatted})] {gender}{ability} - {nature} - {IVList} {size}- {pk.OT_Name}";
        }
